...
- **최신 버전**: v3.6.0 (2026-02-28)
- **최근 수정**: Process Mapping 자동화, 업로드 진행률 UI 개선
...
**목표:** Report 탭 좌측에 Dashboard 탭 추가

**설계 원칙:**
- ✅ 기존 기능 유지 (Regression 방지)
- ✅ 관리자 중심 인사이트 제공
- ✅ 모달 재사용 (Process/Shift/Worker 모달)
- ✅ AI Insight/Warning 메커니즘 (데이터 기반 규칙)

**필수 섹션:**
1. **Trend Visibility**: Utilization/Efficiency 일별/주별 추세 (FO Desc L1/L2/L3 탐색 가능)
2. **Process Comparison**: Top/Bottom 공정 (WoW/DoD 변화율)
3. **Shift View**: Day vs Night 비교
4. **Distribution & Outliers**: 이상 작업자/공정 감지 (신뢰도 라벨)
5. **AI Insight/Warning**: 6-10개 경고 규칙 (급락, 불일치, 데이터 무결성 등)

**구현 계획:**
- [ ] 와이어프레임 설계 (5-8 섹션)
- [ ] Warning Rules 정의 (조건, 메시지, 신뢰도 로직)
- [ ] Modal 스펙 (Process/Shift/Worker)
